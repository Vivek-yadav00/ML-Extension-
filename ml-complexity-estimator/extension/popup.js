        ...
        chrome.tabs.sendMessage(tab.id, { action: "getCode" }, async (response) => {
            ...
            try {
                ...
                const apiResponse = await fetch("http://localhost:8000/api/analyze", {
                    ...
                    body: JSON.stringify({
                        filename: response.filename || "unknown.py",
                        file_content: response.code
                    }),
                ...
                let msg = err.message;
                if (err.name === 'AbortError') msg = "Request timed out. Is the backend running?";
                else if (msg.includes("Failed to fetch")) msg = "Cannot connect to backend. Run 'run_server.bat'.";
...
function displayResults(data) {
    ...
    if (!data.models || data.models.length === 0) {
        resultsDiv.innerHTML = "<p>No ML models detected.</p>";
        return;
    }
    ...
    data.models.forEach(model => {
        ...
        if (model.file_analysis) {
            ...
            if (model.file_analysis.parameters_used && Object.keys(model.file_analysis.parameters_used).length > 0) {
                ...
                html += `<p class="params"><strong>Parameters:</strong> Using defaults</p>`;
            }
